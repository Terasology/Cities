...
public class SimpleTownWallGenerator {
     ...
     */
    public SimpleTownWallGenerator(String seed, Function<BaseVector2i, Integer> heightMap) {
        this.seed = seed;
    ...
    public TownWall generate(City city) {
        ...
        // close the circle
        BaseVector2i firstPoint = tw.getWalls().get(0).getStart();
        ...
        for (WallSegment wall : tw.getWalls()) {
            BaseVector2i center = wall.getStart();
            int towerRad = 3;
            // make sure the width/height are odd to make the BattlementRoof look pretty
            Rectangle layout = new Rectangle(center.getX() - towerRad, center.getY() - towerRad, towerRad * 2 - 1, towerRad * 2 - 1);
